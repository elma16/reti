...
def challenge_from_resigned_games(pgn_file, num_games, tally):  # noqa
    """
...
def challenge_from_endgame_positions(pgn_file, num_games, tally, aim="win"):  # noqa
    ...
    selected_positions = random.sample(
        target_positions, min(num_games, len(target_positions))
    )

    for i, pos in enumerate(selected_positions, 1):
        print(f"\nPosition {i}/{len(selected_positions)}: {pos['game_info']}")
        print(
            f"After move: {pos['move']} ({aim} scenario). Best tablebase move: {pos.get('best_move', 'N/A')}"
        )
        response = requests.post(
            "https://lichess.org/api/challenge/ai",
            headers={"Authorization": f"Bearer {api_key}"},
...
def main_fn():
    parser = argparse.ArgumentParser(
        description="Chess practice tool with resigned games and endgame practice"
    )
    parser.add_argument("--pgn", required=True, help="Path to PGN file")
    parser.add_argument(
        "--mode",
        choices=["resigned", "endgame"],
        default="resigned",
