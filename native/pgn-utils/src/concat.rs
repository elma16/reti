...
//!
//! Inputs may be files, directories, or a lone `-` for stdin; directories are
//! walked recursively for `*.pgn` (case-insensitive) and visited in
//! deterministic, sorted order so output is reproducible. Output goes to
//! `--output PATH`/`-o PATH`, to `-o -` (forced stdout), or — when stdout is
//! not a terminal — straight to stdout so it can be piped:
//!
//!   pgn-utils concat . | pgn-utils clean - | pgn-utils dedup - -o final.pgn
//!
//! When the data stream goes to stdout the trailing JSON stats line is written
//! to stderr instead, so it never corrupts a downstream parser.
//!
//! Optional flags:
...
/// `.pgn` files, and the result is sorted for stability. A lone `-` (stdin)
/// is passed through untouched and never sorted into the file list ahead of
/// the explicit files (it keeps its place at the front).
pub fn expand_inputs(inputs: &[PathBuf]) -> io::Result<Vec<PathBuf>> {
    let mut stdin_inputs: Vec<PathBuf> = Vec::new();
    let mut out: Vec<PathBuf> = Vec::new();
    for path in inputs {
        if crate::output::is_stdin_path(path) {
            stdin_inputs.push(path.clone());
            continue;
        }
...
/// Run a concat. Returns the stats and whether the stats line should be
/// printed to stderr (true when the data stream went to stdout).
pub fn run_concat(opts: ConcatOptions) -> io::Result<(ConcatStats, bool)> {
    ...
    // Resolve the destination up front so the terminal-flood guard fires
    // before we draw a progress bar or touch any inputs.
    let sink = crate::output::open_output(opts.output.as_deref(), "concat")?;
    ...
    for path in &files {
        ...
        if opts.clean {
            ...
            // Dedup but no clean: we need the full bytes for emit_games_with_dedup
            // so we can preserve each game's exact text. Read into a Vec then split.
            let (reader, _len) = crate::output::open_input(path)?;
            let mut reader = progress.wrap(reader);
            let mut splitter_buf: Vec<u8> = Vec::new();
            reader.read_to_end(&mut splitter_buf)?;
            stats.bytes_in += splitter_buf.len() as u64;
            ...
            let (games_in_file, bytes_read) = stream_normalized(reader, &mut writer)?;
            stats.games_written += games_in_file;
            stats.bytes_in += bytes_read;
        }
    }
...
/// Returns `(games, bytes_read)`: an estimate of games (number of `[Event `
/// lines seen) and the raw byte count consumed from `reader` (so callers can
/// report `bytes_in` even for stdin, whose size is unknown up front).
fn stream_normalized<R: BufRead, W: Write>(mut reader: R, out: &mut W) -> io::Result<(usize, u64)> {
...
fn emit_raw_normalized<W: Write>(bytes: &[u8], out: &mut W) -> io::Result<()> {
    stream_normalized(Cursor::new(bytes), out).map(|(_games, _bytes)| ())
}
...
pub fn run_subcommand(args: &[OsString]) -> Result<(), String> {
    ...
    let (stats, stats_to_stderr) = run_concat(opts).map_err(|e| format!("concat failed: {e}"))?;
...
mod tests {
    ...
    fn stream_normalized_collapses_blank_lines() {
        let input = b"[Event \"a\"]\n[Result \"1-0\"]\n\n\n\n1. e4 1-0\n";
        let mut out = Vec::new();
        let (games, bytes_read) = stream_normalized(Cursor::new(input), &mut out).unwrap();
