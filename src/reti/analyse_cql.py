...
def run_cql_analysis(pgn_file, cql_binary, cql_scripts, output_dir):
    ...
    for script_path in cql_scripts:
        ...
        try:
            ...
            print(
                f"An unexpected error occurred while processing {script_path.name}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run CQL scripts against a PGN file and report game counts."
    )
    ...
    parser.add_argument(
        "scripts_location",
        help="Path to a single .cql script or a directory containing .cql scripts.",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        default=None,
        help="Directory to store temporary output PGN files (optional, default is a temporary directory).",
    )
    parser.add_argument(
        "--keep_output",
        action="store_true",
        help="Keep the temporary output PGN files after processing.",
    )
    ...
    if args.output_dir:
        ...
        )  # Only clean up temp dir if --keep_output is not set
        print(f"Using temporary output directory: {output_directory}")

    stats = run_cql_analysis(
        args.pgn_file, args.cql_binary, cql_scripts, output_directory
    )
    ...
    if cleanup_needed:
        try:
            ...
            print(
                f"Warning: Could not remove temporary directory {output_directory}: {e}"
            )
