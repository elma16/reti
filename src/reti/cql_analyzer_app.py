...
app.config['MAX_CONTENT_LENGTH'] = None  # No file size limit - process what fits in memory
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_script):
            ...
            try:
                ...
                # Progress indicator
                print(f"Progress: {completed}/{len(cql_scripts)} scripts completed ({completed*100//len(cql_scripts)}%)")
...
def analyze():
    ...
    try:
        ...
        print(f"Analysis complete: {len(sorted_results)} ending types, {total_positions} positions")
...
if __name__ == '__main__':
    ...
    if not TEMPLATE_DIR.exists():
        ...
        print(f"Please create a 'templates' folder in the same directory as this script")
        print(f"and place index.html inside it.")
        exit(1)
    
    if not (TEMPLATE_DIR / 'index.html').exists():
        print(f"ERROR: index.html not found in {TEMPLATE_DIR}")
        print(f"Please make sure index.html is in the templates folder")
        exit(1)
